...
namespace FlowCanvas.Nodes{
	...
	abstract public class EventNode : FlowNode {
		...
		public override string name{
			get {return string.Format("<color=#ff5c5c>➥ {0}</color>", base.name.ToUpper());}
		}
	}
	...
	abstract public class EventNode<T> : EventNode where T:Component{
		...
		public override string name{
			get {return string.Format("<color=#ff5c5c>{0} ({1})</color>", base.name.ToUpper(), ( target.isNull || target.isNone? "Self" : target.ToString()) );}
		}
