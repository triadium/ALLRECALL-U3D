...
namespace FlowCanvas.Nodes{
	...
	[DoNotList]
	[Description("This is an auto generated Reflection based node")]
	///Converts a MethodInfo into a FlowGraph node
	sealed public class ReflectedMethodNodeWrapper : FlowNode {
		...
		public override string name{
			get
			{
				...
				return "<color=#e22f2f>* Missing Function *</color>";
			}
		}

		private MethodInfo method{
			get {return _method != null && _method.Get() != null? _method.Get() : null;}
		}
		...
		protected override void RegisterPorts(){
			...
			if ( method.DeclaringType.IsValueType || method.GetParameters().Any( p => p.IsOut ) ){
		...
		//Calls the method
		object CallMethod(){
			args = inputs.Select(i => i.value).ToArray();
			if (method.IsStatic){
		...
		protected override UnityEditor.GenericMenu OnContextMenu(UnityEditor.GenericMenu menu){
			if (method != null){
				...
				foreach(var m in method.ReflectedType.GetMethods(BindingFlags.Instance | BindingFlags.Public)){
