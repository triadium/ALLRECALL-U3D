...
namespace FlowCanvas{
	...
	abstract public class FlowNode : Node, ISerializationCallbackReceiver {
		...
		public BinderConnection GetInputConnectionForPortID(string ID){
			return inConnections.OfType<BinderConnection>().Where(c => c.targetPortID == ID).FirstOrDefault();
		}
		...
		public BinderConnection GetOutputConnectionForPortID(string ID){
			return outConnections.OfType<BinderConnection>().Where(c => c.sourcePortID == ID).FirstOrDefault();
		}

		///Returns the first input port assignable to the type provided
		public Port GetFirstInputOfType(Type type){
			return inputPorts.Values.OrderBy(p => p.GetType() == typeof(FlowInput)? 0 : 1 ).Where(p => p.type.RTIsAssignableFrom(type) ).FirstOrDefault();
		}

		///Returns the first output port of a type assignable to the port
		public Port GetFirstOutputOfType(Type type){
			return outputPorts.Values.OrderBy(p => p.GetType() == typeof(FlowInput)? 0 : 1 ).Where(p => type.RTIsAssignableFrom(p.type) ).FirstOrDefault();
		}
		...
		public void AssignSelfInstancePort(){
			...
			if (valueInputs.Length == 0 || graphAgent == null){
				return;
			}
			var instanceInput = valueInputs[0];
		...
		///If RegisterPorts is not overriden, reflection is sued to register ports based on guidelines.
		...
		void ValidateConnections(){
			...
			foreach (BinderConnection cOut in outConnections.ToArray()){ //ToArray because connection might remove itself if invalid
				cOut.GatherAndValidateSourcePort();
			}

			foreach (BinderConnection cIn in inConnections.ToArray()){
				cIn.GatherAndValidateTargetPort();
			}
		}
		...
		sealed public override void DrawNodeConnections(Vector2 canvasMousePos, float zoomFactor){
			...
			GUI.Box(new Rect(nodeRect.x - 8, nodeRect.y + 2, 10, nodeRect.height), "", (GUIStyle)"nodePortContainer");
			GUI.Box(new Rect(nodeRect.xMax - 2, nodeRect.y + 2, 10, nodeRect.height), "", (GUIStyle)"nodePortContainer");
			///

			//INPUT Ports
			if (orderedInputs != null){
				Port instancePort = null;
				var firstValuePort = orderedInputs.OfType<ValueInput>().FirstOrDefault();
				for (var i = 0; i < orderedInputs.Length; i++){
					...
					GUI.Box(portRect, "", port.isConnected? (GUIStyle)"nodePortConnected" : (GUIStyle)"nodePortEmpty");
					...
					if (e.type == EventType.ContextClick && portRect.Contains(e.mousePosition) && port.isConnected){
			...
			///PORT TYPE SENSITIVE CONTEXT MENUS
			if (e.type == EventType.MouseUp && clickedPort != null){
				...
				///ON CANVAS
					...
					if (dragDropMisses == graph.allNodes.Count && clickedPort != null){
						DoContextPortConnectionMenu(clickedPort, e.mousePosition);
						clickedPort = null;
						e.Use();
					}
				}
...
//				var pointerRect = new Rect(e.mousePosition.x + 16, e.mousePosition.y, 120, 22);
//				GUI.Label(pointerRect, string.Format("{0}", clickedPort.name));
			}

			//Actualy draw the existing connections
			for (var i = 0; i < outConnections.Count; i++){
				(outConnections[i] as BinderConnection).DrawConnectionGUI();
			}
		}
		...
		void DoContextPortConnectionMenu(Port clickedPort, Vector2 mousePos){
			...
			if (clickedPort is ValueOutput){
				...
				foreach (var m in methods.OrderBy(m => !m.IsStatic).OrderBy(m => m.IsSpecialName).OrderBy(m => m.DeclaringType != clickedPort.type ) ){
					...
					if (m.IsStatic && !m.GetParameters().Select(p => p.ParameterType).Any(t => t.IsAssignableFrom(clickedPort.type)) ){
						continue;
					}
			...
			if (clickedPort is ValueOutput){
				var types = UserTypePrefs.GetPreferedTypesList(typeof(object), true);
				types.Remove(clickedPort.type);
				foreach (var type in types){
					foreach (var m in type.GetMethods(BindingFlags.Instance| BindingFlags.Static | BindingFlags.Public).OrderBy(m => m.IsSpecialName) ){
						
						var parameters = m.GetParameters();
						if (parameters.Any(p => p.ParameterType.IsAssignableFrom(clickedPort.type)) && !m.IsGenericMethod){
							var categoryName = "Functions/Reflected/" + type.FriendlyName() + "/";
							var name = categoryName + m.SignatureName();
							var icon = UnityEditor.EditorGUIUtility.ObjectContent(null, type).image;
							menu.AddItem(new GUIContent(name, icon), false, SelectedMethod, m);
			...
			if (orderedInputs != null){
				for (var i = 0; i < orderedInputs.Length; i++){
					...
					if (inPort is FlowInput){
						GUILayout.Label("<b>► " + inPort.name + "</b>", leftLabelStyle);
					} else {
						var enumerableList = typeof(IEnumerable).IsAssignableFrom(inPort.type) && (inPort.type.IsGenericType || inPort.type.IsArray);
						GUILayout.Label(string.Format("<color={0}>{1}{2}</color>", UserTypePrefs.GetTypeHexColor(inPort.type), enumerableList? "#" : "" , inPort.name ), leftLabelStyle);
					}
					if (Event.current.type == EventType.Repaint)
						inPort.pos = new Vector2(0, GUILayoutUtility.GetLastRect().center.y + nodeRect.y);
				}
			}
			...
			if (orderedOutputs != null){
				for (var i = 0; i < orderedOutputs.Length; i++){
					...
					if (outPort is FlowOutput){
						GUILayout.Label("<b>" + outPort.name + " ►</b>", rightLabelStyle);
					} else {
						var enumerableList = typeof(IEnumerable).IsAssignableFrom(outPort.type) && (outPort.type.IsGenericType || outPort.type.IsArray);
						GUILayout.Label(string.Format("<color={0}>{1}{2}</color>", UserTypePrefs.GetTypeHexColor(outPort.type), enumerableList? "#" : "", outPort.name), rightLabelStyle);
					}
					if (Event.current.type == EventType.Repaint)
						outPort.pos = new Vector2(0, GUILayoutUtility.GetLastRect().center.y + nodeRect.y);
				}
			}
			GUILayout.EndVertical();
